...
class GiveSubCommands implements SubCommand {
		...
		if(isset($args[0])){
			if(isset($args[1])){
				...
				if(!is_numeric($args[1])){
	                $sender->sendMessage("§l§c•§e Bạn Cần Ghi Số Kim Cương Muốn Tặng");
	                return true;
				}
			    ...
			    $sender->sendMessage("§l§c•§e Đã Lấy§b $diamond Kim Cương§e Cho Người Chơi§a ".$args[0]." §eThành Công !");
			    if($instance->getServer()->getPlayerByPrefix($args[0]) !== null) $p->sendMessage("§l§c•§e Bạn Đã Nhận Được§b $diamond Kim Cương");
			}else{
				$sender->sendMessage("§l§c•§e Sử Dụng:§b /diamond give <player> <diamond>");
			}
		}else{
			$sender->sendMessage("§l§c•§e Sử Dụng:§b /diamond give <player> <diamond>");
		}
